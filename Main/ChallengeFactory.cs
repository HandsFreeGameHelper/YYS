...
public static class ChallengeFactory
{
  ...
  private static bool  ExcuteEnvent(this Random random, string path, Bitmap scaledBmp, Point elementPosition, Point eventPosition)
  {
    using (Bitmap eventBmp = new Bitmap(path))
    {
      ...
      {
        random.MoveAndClick(eventPosition,random.Next(5,10));
        return true;
      }
      return false;
    }
